											...
											<tr>
												...
												<td>
													<center>
														...
														<?php }
														if ($this->permission->check_label('manage_category')->delete()->access()) { ?>
															<!-- F9GNCBBPCOIEN67 -->
															<?php if($category['category_id'] !== 'F9GNCBBPCOIEN67'){  ?>
															<a href="<?php echo base_url('dashboard/Ccategory/category_delete/' . $category['category_id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('<?php echo display('are_you_sure_want_to_delete') ?>');" data-toggle="tooltip" data-placement="right" title="" data-original-title="<?php echo display('delete') ?> ">
